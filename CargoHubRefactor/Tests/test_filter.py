...
def test_admin_token_full_access(_data):
    ...
    response = requests.post(url, headers=headers, json={"example": "data"})
...
def test_floor_manager_token_put_limited_paths(_data):
    ...
    url_not_allowed = _data["URL"] + "Warehouses"
...
def test_warehouse_manager_token_put_limited_paths(_data):
    ...
    url_not_allowed = _data["URL"] + "Orders"
...
def test_missing_token(_data):
    ...
    response = requests.post(url, json={"example": "data"})
